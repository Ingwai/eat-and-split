	...
	return (
		<div className='app'>
			<div className='sidebar'>
				<FriendsList friends={friends} selectedFriend={selectedFriend} onSelectedFriend={selectionHandler} />

				{showAddFriend && <FormAddFriend onAddFriends={addFriendHandler} />}

				<Button onClick={showAddFriendHandler}>{showAddFriend ? 'Close' : 'Add friend'}</Button>
			</div>

			{selectedFriend && (
				<FormSplitBill selectedFriend={selectedFriend} onSplitBill={splitBillHandler} key={selectedFriend.id} />
			)}
		</div>
	);
}
...
const Friend = ({ friend, onSelectedFriend, selectedFriend }) => {
	...
	return (
		<li className={isSelected ? 'selected' : ''}>
			...
			{friend.balance < 0 && (
				<p className='red'>
					You owe {friend.name} {Math.abs(friend.balance)} PLN
				</p>
			)}
			{friend.balance > 0 && (
				<p className='green'>
					{friend.name} owes you {Math.abs(friend.balance)} PLN
				</p>
			)}
...
const FormAddFriend = ({ onAddFriends }) => {
	...
	const handleSubmit = e => {
		...
		const id = crypto.randomUUID(); //wbudowany w przeglądarkę do tworzenia ID
		const newFriend = {
			id: id,
			name: name,
			image: `${image}?=${id}`,
